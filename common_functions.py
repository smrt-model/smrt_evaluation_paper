...
def symmetrize_microstructure(file, direction=None):
    ...
    if direction is None:
        ...
        # y-component assumed to be same as x-component
        # Take mean, and change units to m
        l_c = df[' 3:l_c_x'].mul(1e-3)
        l_ex = df[' 9:l_ex_x'].mul(1e-3)
        d_sph = df[' 6:d_sph_x'].mul(1e-3) 
        xi_ts = df[' 12:xi_ts_x'].mul(1e-3) 
        domain_ts = df[' 15:d_ts_x'].mul(1e-3)  
        xi_grf = df[' 18:xi_grf_x'].mul(1e-3)   
        domain_grf = df[' 21:d_grf_x'].mul(1e-3)
    elif direction == 'z':
        density = df[' 2:phi_2'].mul(DENSITY_OF_ICE)  # Density
        d_shs = df[' 24:d_shs'].mul(1e-3)  # Change units to m
        tau = df[' 25:tau'] # Unitless
        # y-component assumed to be same as x-component
        # Take mean, and change units to m
        l_c = df[' 5:l_c_z'].mul(1e-3)
        l_ex = df[' 11:l_ex_z'].mul(1e-3)
        d_sph = df[' 8:d_sph_z'].mul(1e-3) 
        xi_ts = df[' 14:xi_ts_z'].mul(1e-3) 
        domain_ts = df[' 17:d_ts_z'].mul(1e-3)  
        xi_grf = df[' 20:xi_grf_z'].mul(1e-3)   
        domain_grf = df[' 23:d_grf_z'].mul(1e-3)
    else:
        print ('direction not recognized. Amend code to include y if needed')
        return
...
# NB image checked with https://www.color-blindness.com/coblis-color-blindness-simulator/
def microstructure_colour_list():
    return {'Exponential': 'r', 'IndependentSphere':'b', 'StickyHardSpheres':'c', 'TeubnerStrey':'violet', 'GaussianRandomField':'y'}
...
def get_microstructure_fits(file, direction=None):
    df = pd.read_csv(file)
    if direction is not None:
        dirn = '_' + direction
        return df.filter(regex='rmse').filter(regex=dirn)
    else:
        return df.filter(regex='rmse').filter(regex=('_x|_z'))


def microstructure_symbols():
    return {'Exponential': 'X', 'IndependentSphere':'o', 'StickyHardSpheres':'s', 'TeubnerStrey':'v', 'GaussianRandomField':'d'}
